...
def _batch_normalization(name, x, d_next, is_conv=True):
...
def _conv2d(layer_name, x, k, s, d_next):
    ...
    with tf.variable_scope(layer_name):
        ...
        batch = _batch_normalization('2_batch_norm', conv, d_next)
        relu = tf.nn.relu(batch, name='3_relu')
        _variable_summaries(relu)
        return relu

def _fc(layer_name, x, d_next):
    """fully-connected layer"""
    with tf.variable_scope(layer_name):
        batch_size = x.get_shape()[0].value
        x_flat = tf.reshape(x, [batch_size, -1])
        num_v_prev = x_flat.get_shape()[1].value
        W = _weight_variable('weights', [num_v_prev, d_next])
        fc = tf.matmul(x_flat, W, name='1_matmul')
        _variable_summaries(fc)
        batch = _batch_normalization('2_batch_norm', fc, d_next, is_conv=False)
        relu = tf.nn.relu(batch, name='3_relu')
        _variable_summaries(relu)
        return relu        
...
def inference(images):
    ...
    h_conv11 = _conv2d('1-1_down', images, 3, 2, 64)
    ...
    h_conv21 = _conv2d('2-1_down', h_conv12, 3, 2, 128)
    ...
    h_conv31 = _conv2d('3-1_down', h_conv22, 3, 2, 256)
    ...
    h_conv41 = _conv2d('4-1_down', h_conv32, 3, 2, 512)
    # 4-2 flat-convolutional layer: k=3x3, s=1x1, d=512
    h_conv42 = _conv2d('4-2_flat', h_conv41, 3, 1, 512)

    # 5-1 fully-connected layer: d=1024
    h_fc51 = _fc('5-1_fc', h_conv42, 1024)
    # 5-2 fully-connected layer: d=512
    h_fc52 = _fc('5-2_fc', h_fc51, 512)
    # 5-3 fully-connected layer: d=256
    h_fc53 = _fc('5-3_fc', h_fc52, 256)
    # 5-4 fully-connected layer: d=8
    y_fc = _fc('5-4_fc', h_fc53, 8)
