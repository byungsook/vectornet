...
from linenet.linenet_manager_line import LinenetManager
from linenet.linenet_manager_ov_line import IntersectnetManager
from linenet.linenet_data_line_gc import BatchManager
...
def _compute_accuracy(svg, labels, line_pixels, num_line_pixels, dup_rev_dict):
    stroke_list = []
    ...
    if system_name is not 'Windows':
        ...
        for i in xrange(num_paths):
            svg_xml = ET.fromstring(svg)
            for j in reversed(xrange(num_paths)):
...
def graphcut(linenet_manager, intersectnet_manager, batch_manager, id):
    file_name = str(id)
    ...
    accuracy_list = _compute_accuracy(svg, labels, line_pixels, num_line_pixels, dup_rev_dict)
    ...
    for i in xrange(FLAGS.max_num_labels):
        ...
        if system_name is not 'Windows':
            ...
            call(['data_tmp/potrace-1.13.win64/potrace.exe', '-s', '-i', '-C'+color_hex, i_label_map_path])
...
def test():
    ...
    for i in xrange(FLAGS.num_test_files):
        start_time = time.time()
        num_labels, diff_labels, acc_avg = graphcut(linenet_manager, intersectnet_manager, batch_manager, i)
        duration = time.time() - start_time
        print('%s:%d/%d processed (%.3f sec)' % (datetime.now(), i, FLAGS.num_test_files, duration))
        sf.write('%d %d %d %.3f %.3f\n' % (i, num_labels, diff_labels, acc_avg, duration))
