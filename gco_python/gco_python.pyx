...
cdef extern from "GCoptimization.h":
    ...
    cdef cppclass GCoptimizationGeneralGraph:
        ...
        void setLabelCost(int label_cost) except +
        void expansion(int n_iterations) except +
        void swap(int n_iterations) except +
        int whatLabel(int node) except +
        int compute_energy() except +
        int giveDataEnergy() except +
        int giveSmoothEnergy() except +
        int giveLabelEnergy() except +
...
def cut_from_graph(np.ndarray[np.int32_t, ndim=2, mode='c'] edges,
        ...
        np.ndarray[np.int32_t, ndim=2, mode='c'] pairwise_cost, label_cost=1000, n_iter=5,
        algorithm='expansion'):
    """
    ...
    pairwise_cost: ndarray, int32, shape=(n_labels, n_labels)
        Pairwise potentials for label compatibility
    label_cost: int, (default=1000)
        Global potentials for indicator        
    n_iter: int, (default=5)
        Number of iterations
    algorithm: string, `expansion` or `swap`, default=expansion
        Whether to perform alpha-expansion or alpha-beta-swaps.
    ...
    # raise ValueError('make test')

    cdef int energy_before[3]
    # gc.giveDataEnergy()
    energy_before[0] = gc.giveSmoothEnergy()
    energy_before[1] = gc.giveLabelEnergy()
    energy_before[2] = gc.compute_energy()
    # print(energy_before[0], energy_before[1], energy_before[2])      
    ...
    cdef int energy_after[3]
    # gc.giveDataEnergy()
    energy_after[0] = gc.giveSmoothEnergy()
    energy_after[1] = gc.giveLabelEnergy()
    energy_after[2] = gc.compute_energy()
    # print(energy_after[0], energy_after[1], energy_after[2])   
    ...
    return result, energy_before, energy_after