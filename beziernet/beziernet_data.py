...
def train_set(i, x_batch, y_batch):
    ...
    if xy[0] > xy[6] or (xy[0] == xy[6] and xy[1] > xy[7]):
        _reverse_order(xy)

    SVG_LINE = SVG_START_TEMPLATE.format(
            width=FLAGS.image_size,
            height=FLAGS.image_size
        ) + SVG_CUBIC_BEZIER_TEMPLATE.format(
            id=0,
            sx=xy[0], sy=xy[1],
            cx1=xy[2], cy1=xy[3],
            cx2=xy[4], cy2=xy[5],
    ...
    # save x png
    x_png = cairosvg.svg2png(bytestring=SVG_LINE)
    x_img = Image.open(io.BytesIO(x_png))
    
    # load and normalize x to [0, 1]
    ...
    y_batch[i,:] = xy.astype(np.float)


def batch():
    x_batch = np.empty([FLAGS.batch_size, FLAGS.image_size, FLAGS.image_size, 1], dtype=np.float)
    y_batch = np.empty([FLAGS.batch_size, FLAGS.xy_size], dtype=np.float)
    for i in xrange(FLAGS.batch_size):
        train_set(i, x_batch, y_batch)
...
def svg_to_png(xy, num_image):
    ...
    img_batch = np.empty([num_image, FLAGS.image_size, FLAGS.image_size, 1], dtype=np.uint8)
    for i in xrange(num_image):
        ...
        png = cairosvg.svg2png(bytestring=SVG_LINE)
        img = Image.open(io.BytesIO(png))
        img_batch[i,...] = np.reshape(np.array(img)[:,:,3], [FLAGS.image_size, FLAGS.image_size, 1])
