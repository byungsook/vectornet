...
def batch_normalization(conv, d_next):
    ...
    return tf.nn.batch_normalization(conv, mean, var, scale, offset, 1e-3)
...
# up-convolution
def up_conv2d(layer_name, x, k, s, d_next):
    with tf.name_scope(layer_name):
        _, h, w, d_prev = x.get_shape()
        batch_size, h, w, d_prev = tf.shape(x)[0], h.value, w.value, d_prev.value
        batch_size, h, w = tf.shape(x)[0], tf.shape(x)[1], tf.shape(x)[2]
        with tf.name_scope('weights'):
            W = weight_variable([k, k, d_next, d_prev])
            # variable_summaries(W, layer_name + '/weights')
        # o = [batch_size, h*2, w*2, d_next]
        o = tf.pack([batch_size, h*2, w*2, d_next])
        b = bias_variable([d_next])
...
def main():
    ...
    # 1-2 flat-convolutional layer: k=3x3, s=1x1, d=512
    ...
    y_conv = conv2d('1-4_flat', h_conv13, 3, 1, 1)
    ...
    # h_conv41 = up_conv2d('4-1_up', h_conv38, 4, 2, 256)
    ...
    # h_conv51 = up_conv2d('5-1_up', h_conv43, 4, 2, 256)
    ...
    # h_conv61 = up_conv2d('6-1_up', h_conv53, 4, 2, 48)
